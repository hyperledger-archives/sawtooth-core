...
class TestNetworkPermissioning(unittest.TestCase):
    ...
    def test_network_trust_permissioning(self):
        ...
        Notes:
            ...
            3) Add a policy that denies the non-genesis validator from the
               network role.
            4) Assert that the validators get 3 blocks out of consensus.
        ...
        wait_for_out_of_consensus(self.clients)
    ...
    def test_network_consensus_permissioning(self):
        """Test the network.consensus "trust" permissioning role.
        ...
        Notes:
            ...
            4) Assert that the validators get 3 blocks out of consensus.
            5) Add a new policy that allows both validators to use the
               network consensus role.
            6) Assert that the validators come back into consensus within 200s.

        """

        walter = Admin("http://127.0.0.1:8084")
        ...
        with SetSawtoothHome(sawtooth_home0):
            write_validator_config(
                sawtooth_home0,
                roles={"network": "trust"},
                endpoint="tcp://127.0.0.1:8804",
                bind=["network:tcp://127.0.0.1:{}".format(8804),
                      "component:tcp://127.0.0.1:{}".format(4008)],
                seeds=["tcp://127.0.0.1:8805"],
                peering="dynamic",
                scheduler='parallel')
            validator_non_genesis_init(sawtooth_home1)
            validator_genesis_init(
                ...
                identity_pub_key=walter.pub_key,
                role="network")
            processes.extend(start_validator(4, sawtooth_home0))
            self.clients.append(Client("http://127.0.0.1:8084"))

        with SetSawtoothHome(sawtooth_home1):
            write_validator_config(
                sawtooth_home1,
                roles={"network": "trust"},
                endpoint="tcp://127.0.0.1:8805",
                bind=["network:tcp://127.0.0.1:{}".format(8805),
                      "component:tcp://127.0.0.1:{}".format(4009)],
                peering="dynamic",
                seeds=["tcp://127.0.0.1:8804"],
                scheduler='parallel')

            processes.extend(start_validator(5, sawtooth_home1))

            self.clients.append(Client("http://127.0.0.1:8085"))
    ...
    def test_network_consensus_challenge_permissioning(self):
        """Test the network.consensus "challenge" permissioning role.
        ...
        walter = Admin("http://127.0.0.1:8086")
        ...
        with SetSawtoothHome(sawtooth_home0):
            write_validator_config(
                sawtooth_home0,
                roles={"network": "challenge"},
                endpoint="tcp://127.0.0.1:8806",
                bind=["network:tcp://127.0.0.1:{}".format(8806),
                      "component:tcp://127.0.0.1:{}".format(4010)],
                seeds=["tcp://127.0.0.1:8807"],
                peering="dynamic",
                scheduler='parallel')
            validator_non_genesis_init(sawtooth_home1)
            validator_genesis_init(
                ...
                identity_pub_key=walter.pub_key,
                role="network")
            processes.extend(start_validator(6, sawtooth_home0))
            self.clients.append(Client("http://127.0.0.1:8086"))

        with SetSawtoothHome(sawtooth_home1):
            write_validator_config(
                sawtooth_home1,
                roles={"network": "challenge"},
                endpoint="tcp://127.0.0.1:8807",
                bind=["network:tcp://127.0.0.1:{}".format(8807),
                      "component:tcp://127.0.0.1:{}".format(4011)],
               peering="dynamic",
               seeds=["tcp://127.0.0.1:8806"],
               scheduler='parallel')

            processes.extend(start_validator(7, sawtooth_home1))

            self.clients.append(Client("http://127.0.0.1:8087"))
...
def within_tolerance(block_list1, block_list2, tolerance):
    ...
    block_list1.sort(key=lambda x: x[0], reverse=True)
    block_list2.sort(key=lambda x: x[0], reverse=True)
