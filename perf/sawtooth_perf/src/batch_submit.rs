...
pub fn run_workload(batch_list_iter: &mut Iterator<Item = BatchListResult>, time_to_wait: u32, update_time: u32, targets: Vec<String>, basic_auth: Option<String>)
    -> Result<(), WorkloadError>
{
    ...
    let basic_auth_clone = basic_auth.clone();
    ...
    let stream_work = stream
        ...
        .map(|batch_list| {
            ...
            match req {
                ...
                let basic_auth_clone2 = basic_auth_clone.clone();
                ...
                let work = client.request(req)
                    ...
                    .and_then(move |uri: Result<Uri, WorkloadError> | {
                        match uri {
                            Ok(uri) => {
                                let url: Uri = uri;
                                let mut req: Request<Body> = Request::new(Method::Get, url);
                                match basic_auth_clone2 {
                                    Some(ref basic_auth) => {
                                        req.headers_mut().set(Authorization(
                                            Basic::from_str(&basic_auth)?
                                        ));
                                ...
                                let get_status = client_clone_c.request(req)
