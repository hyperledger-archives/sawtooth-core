...
pub fn run_workload<'a>(batch_list_iter: &'a mut Iterator<Item = BatchListResult>, time_to_wait: u32, update_time: u32, targets: Vec<String>)
    -> Result<(), workload::WorkloadError>
{
    let mut core = Core::new().unwrap();
    let handle = core.handle();
    let client = Rc::new(Client::configure()
        .build(&handle));
    ...
    let stream = interval
        ...
        .map(move |batch_list: Result<BatchList, workload::WorkloadError>| {
            let urls_c = &mut urls;
            workload::form_request_from_batchlist(urls_c, batch_list)
        })
        .map_err(|err| workload::WorkloadError::from(err))
