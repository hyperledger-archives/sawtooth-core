...
pub fn run_workload(batch_list_iter: &mut Iterator<Item = BatchListResult>, time_to_wait: u32, update_time: u32, target: String)
    -> Result<(), WorkloadError>
{
    ...
    let mut batch_url = target.clone();
    batch_url.push_str("/batches");
    let url_for_error = batch_url.clone();
    ...
    let stream_work = stream
        .map(move |_| {
            if counter_l.has_url_error() {
                return Err(WorkloadError::from(io::Error::new(io::ErrorKind::Other, format!("Could not access url, {}", url_for_error))))
            }
