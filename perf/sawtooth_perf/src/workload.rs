...
impl fmt::Display for WorkloadError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            WorkloadError::Utf8Error(ref err) => {
                write!(f, "A utf8 decoding error occurred: {}", err)
            }
            ...
            WorkloadError::JsonError(ref err) => {
                write!(f, "A json parsing error occurred: {}", err)
            }
...
impl fmt::Display for HTTPRequestCounter {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "{0}, Sent: {1}, Committed {2}/{1}, Invalid {3}/{1} Queue Full {4}/{1}",
...
fn resolve_future_response_body(response: Response) -> Collect<Body> {
    response.body().collect()
}

/// Create a string out of the bytes in the Chunks.
fn collect_future_bytes_to_string(chunks: Vec<Chunk>) -> Result<String, WorkloadError> {
    ...
    match req {
        Ok(req) => {
            ...
            let response_future = client
                ...
                .and_then(|response: Response| resolve_future_response_body(response))
                .map(|chunks: Vec<Chunk>| collect_future_bytes_to_string(chunks))
...
fn handle_future_post_response(
    counter: Rc<HTTPRequestCounter>,
...
fn form_request_from_batches_response(
    basic_auth: Option<String>,
...
fn handle_getting_batch_status(
    client: Rc<Client<HttpConnector>>,
    handle: Handle,
    counter: Rc<HTTPRequestCounter>,
    req: Result<Request, WorkloadError>,
...
fn get_future_batch_status_response_json(
    json_string: Result<String, WorkloadError>,
) -> Option<BatchStatusesResponse> {
    match json_string {
        Ok(s) => match serde_json::from_str::<BatchStatusesResponse>(&s) {
