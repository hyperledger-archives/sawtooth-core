...
from sawtooth_seg.seg_communication import SegCommunication
from sawtooth_seg.seg_communication import MessageException
from sawtooth_seg.seg_state import SegState
from sawtooth_seg.txn_family import SegTransaction
from sawtooth_seg.txn_family import SegTransactionMessage
from sawtooth_seg.seg_exceptions import SegClientException

LOGGER = logging.getLogger(__name__)


class SegClient(SegCommunication):
    def __init__(self,
                 baseurl,
                 name='SegClient',
                 keystring=None,
                 keyfile=None,
                 state=None):
        super(SegClient, self).__init__(baseurl)
        self.LastTransaction = None

        self._current_state = state or SegState(self.BaseURL)
    ...
    def _sendtxn(self, minfo):
        ...
        txn = SegTransaction(minfo=minfo)
        ...
        if not txn.is_valid(self._current_state.State):
            raise SegClientException('transaction failed to apply')

        msg = SegTransactionMessage()
        msg.Transaction = txn
