...
class BatchInjector(metaclass=abc.ABCMeta):
    ...
    def before_batch(self, previous_block, batch):
        """Returns an ordered list of batches to inject before a batch.
        Can also return None if no batches should be injected.
    ...
    def after_batch(self, previous_block, batch):
        """Returns an ordered list of batches to inject after a batch.
        Can also return None if no batches should be injected.
    ...
    def block_end(self, previous_block, batches):
        """Returns an ordered list of batches to inject at the end of the
        block. Can also return None if no batches should be injected.
...
class DefaultBatchInjectorFactory:
    ...
    def _create_injector(self, injector):
        ...
        if injector == "block_end":
            block_end_injector = importlib.import_module(
                "sawtooth_validator.journal.injectors."
                "block_end_block_info_injector")
            return block_end_injector.BlockEndBlockInfoInjector(
                self._state_view_factory, self._signer)
        if injector == "before_batch":
            before_batch_injector = importlib.import_module(
                "sawtooth_validator.journal.injectors."
                "before_batch_example_injector")
            return before_batch_injector.BeforeBatchExampleInjector(
                self._state_view_factory, self._signer)
        if injector == "after_batch":
            after_batch_injector = importlib.import_module(
                "sawtooth_validator.journal.injectors."
                "after_batch_example_injector")
            return after_batch_injector.AfterBatchExampleInjector(
                self._state_view_factory, self._signer)
