...
mod ffi {
    ...
    extern "C" {
        pub fn load_pem_key(
            ...
            pemstr_len: size_t,
            password: *mut c_char,
            out_priv_key: *mut c_char,
            out_pub_key: *mut c_char,
        ) -> c_int;
    }
...
pub fn load_pem_key(pemstr: &str, password: &str) -> Result<(String, String), Error> {
    ...
    match err_num {
        -1 => Err(Error::ParseError(String::from(
            "Failed to decrypt or decode private key",
        ))),
        -2 => Err(Error::ParseError(String::from(
            "Failed to create new big number context",
        ))),
        -3 => Err(Error::ParseError(String::from("Failed to load group"))),
        -4 => Err(Error::ParseError(String::from(
            "Failed to load private key",
        ))),
        -5 => Err(Error::ParseError(String::from(
            "Failed to load public key point",
        ))),
        -6 => Err(Error::ParseError(String::from(
            "Failed to construct public key from point",
        ))),
        _ => {
            let priv_key = c_out_priv_key
                .into_string()
                .map_err(|_| Error::ParseError(String::from("FFI Error")))?;
            let pub_key = c_out_pub_key
                .into_string()
                .map_err(|_| Error::ParseError(String::from("FFI Error")))?;
            Ok((priv_key, pub_key))
        }
