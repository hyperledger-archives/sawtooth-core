...
const _handleCreate = (xo_state, payload, player) => (game) => {
  ...
  return xo_state.setGame(payload.name, createdGame)
}

const _handleTake = (xo_state, payload, player) => (game) => {
  let name = payload.name
  ...
  return xo_state.setGame(payload.name, game)
}

const _handleDelete = (xo_state, payload) => (game) => {

  if (game === undefined) {
    throw new InvalidTransaction(
      `No game exists with name ${payload.name}: unable to delete`)
  }
  return xo_state.deleteGame(payload.name)
  ...
  apply (transactionProcessRequest, context) {
    return new XoPayload().fromBytes(transactionProcessRequest.payload)
      ...
      .then((payload) => {
        ...
        let xo_state = new XoState(context)
        ...
        return xo_state.getGame(payload.name)
          .then(handlerFn(xo_state, payload, player))
      })
  }
}
