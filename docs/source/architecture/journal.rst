...
1. ** Permission and Validation** - The following checks must succeed:

   * On-chain transaction permissions are checked to see who is allowed to
     submit transactions and batches.

   * On-chain block validation rules are checked to ensure that the Block
     doesn't invalidate any of the on-chain validation rules stored at
